...
def prob2rate(p,tstep=1):
    return np.absolute(-np.log(1 - p) / tstep)

def rate2prob(r,tstep=1):
    return 1 - np.exp(-r * tstep)

def get_rate(system,element,tstep=1):
    if element.tag == 'ref': element = system.xpath(f'..//node[@name="{element.get("name")}"]')[0]
    if element.get('rate','-1') == '-1':
        r = [float(get_rate(system,child)) for child in element]
        p = [rate2prob(x,tstep) for x in r]
        if element.get('gate') == 'OR': p = bayes.OR(p)
        if element.get('gate') == 'AND': p = bayes.AND(p)
        if element.get('gate') == 'VOTE': p = bayes.VOTE(p,float(element.get('k')))
        element.set('rate',str(prob2rate(p,tstep)))
    return element.get('rate')
...
def newFT(system,root=None,file='diagram.png',probs=None,rates=None,tstep=1,color=False,scale=2):
    ...
    if probs is not None:
        ...
        for event in events: event.set('prob',str(np.around(probs[event.get('name')],decimals)))
        get_prob(system,root)
    if rates is not None:
        for element in nodes + events: element.set('rate','-1')
        for event in events: event.set('rate',str(np.around(rates[event.get('name')],decimals)))
        get_rate(system,root,tstep)
    index = None
    if len(nodes + events) > 20: index = {}; i = 0
    with schemdraw.Drawing(file=file,show=False) as d:
        for event in events:
            ...
            if probs is not None:
                p = np.around(float(event.get('prob')),decimals)
                label += '\np=' + str(p)
                if color: fill = (1,1-p/2,1-p/2,p/2)
            if rates is not None:
        ...
        if index is not None:
            ...
            y = pos[root][1]
            d += elements.Line().at((x,y)).right().label('#    Element',halign='left')
            d += elements.Line().right()
            for key in index.keys():
                y -= spacing[1] / 5
                d += elements.Label().at((x,y)).label(f'{str(index[key])}    {key}',halign='left')
